...
autoTune_distance <- function(V, n, hs, D, alpha,  delta = 0.05, B = 10, trace = T, lambda = 1, func){
  n0 = length(V)
  J = length(hs)
  n1 = min(n0, n+1)
  if(n1>= 2/3 * n0){
                       ...
                       LCP_auto_tune = function(V0, H0, hs, B = 5, delta = 0.05, lambda = 0, trace = TRUE){
                         if(self$type == "distance"){
                           ret = autoTune_distance(V = V0, n = self$n, hs = hs, D = H0, alpha = self$alpha,  
                                                   delta =delta, B = B, trace = trace, lambda = lambda, func=self$weighted_cumsum)
                         }else{
